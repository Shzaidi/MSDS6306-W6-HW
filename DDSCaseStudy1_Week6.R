...
# DataCamp Light installation and attach : https://github.com/Shzaidi/tutorial
...
dim(EduData)[1] 
str(EduData)
# This gives us the number of countries for which we have EduData data for (eg 234)
...
#        Ensuring GDP is captured as numeric vector and is not 
#        in scientific format in the data frame, and 
#        remove "NA" as part of cleaning process :
...
message("The average ranking for the High Income OECD countries is: ", round(answ3.1, digits = 2))

answ3.2 <- with(Data3, mean(Ranking[Income_Group == "High income: nonOECD"]))
message("The average ranking for the High Income Non-OECD countries is: ", round(answ3.2, digits = 2))
...
legend(x = "topleft", cex = 0.7, c(" Low income in Grey ",
                               ...
                               " High income ??? NonOECD in Red ",
                               " High income ??? OECD in Black "))


# Formatting and adding text / labels to this last chart as above (uses library plotrix)
...
# ********************************************************************************************
# Which countries do we not have both GDP and EDU Data for?
...
#   Implementing a double loop by column to check for equal values in two tables and 
#   write check results in a new DF table with Country Names and Check columns:
